...
namespace forma
{
	...
	Token Token::withType(TokenType new_type) const { return {new_type, Lexeme, Location, Value}; }
	Token Token::withValue(const std::string& new_value) const { return {Type, Lexeme, Location, new_value}; }
	...
	struct Scanner
	{
		...
		std::optional<Token> ScanCodeToken()
		{
			...
			default:
				...
				else if (IsAlpha(c))
				{
					...
					// check keywords
					if(ident.Value == "if") { ident.Type = TokenType::KeywordIf; return ident; }
					if(ident.Value == "range") { ident.Type = TokenType::KeywordRange; return ident; }
					if(ident.Value == "end") { ident.Type = TokenType::KeywordEnd; return ident; }
					if (ident.Value == "include") { ident.Type = TokenType::KeywordInclude; return ident; }
					return ident;
				}
				else
				{
		...
		ScannerLocation NextChar() { return with_index_offset(current, current.Index + 1,current.Offset + 1); }
		...
		Token CreateToken(TokenType tt, std::optional<std::string> value = std::nullopt, std::optional<ScannerLocation> begin = std::nullopt, std::optional<ScannerLocation> end = std::nullopt)
		{
	...
	std::pair<std::vector<Token>, std::vector<Error>> Scan(const std::string& file, const std::string& source)
	{
