...
namespace forma
{
    ...
    FuncGenerator OptionalStringArgument(std::function<std::string(const std::string&, const std::string&)> f, std::string missing)
    {
        return [f, missing](const Location& location, const std::vector<FuncArgument>& args)
        {
            if (args.size() == 0)
            {
                ...
                return FuncGeneratorResult([f, first_arg](const std::string& arg) { return f(arg, first_arg); }, NoErrors());
            }
            else {
        ...
        return [f, missing](const Location& location, const std::vector<FuncArgument>& args)
        {
            ...
            else if (args.size() == 1)
            {
                ...
                if(number.has_value()) {
                    ...
                    return FuncGeneratorResult([f, the_number](const std::string& arg) {
                        return f(arg, the_number); }, NoErrors());
                }
                else {
                    return SyntaxError({
    ...
    FuncGenerator StringStringArgument(std::function<std::string (std::string, std::string, std::string)> f)
    {
        return [f](const Location& location, const std::vector<FuncArgument>& args)
        {
            if (args.size() != 2)
            ...
            return FuncGeneratorResult{ [f, args](const std::string& arg) -> std::string { return f(arg, args[0].Argument, args[1].Argument); }, NoErrors()};
        };
    }
    ...
    FuncGenerator IntIntArgument(std::function<std::string (std::string, int, int)> f)
    {
        return [f](const Location& location, const std::vector<FuncArgument>& args)
        {
            ...
            return FuncGeneratorResult{ [f, lhs, rhs](const std::string& arg) -> std::string { return f(arg, *lhs, *rhs); }, NoErrors() };
        };
    }
    ...
    std::unordered_map<std::string, FuncGenerator> DefaultFunctions()
    {
        ...
        t.insert({"capitalize", NoArguments([](const std::string& args) { return strings::Capitalize(args, true);})});
        ...
        t.insert({"title", NoArguments([](const std::string& args) { return strings::ToTitleCase(args);})});

        t.insert({"rtrim", OptionalStringArgument([](const std::string& str, const std::string& spaceChars) { return strings::TrimEnd(str, spaceChars);}, strings::default_space())});
        t.insert({"ltrim", OptionalStringArgument([](const std::string& str, const std::string& spaceChars) { return strings::TrimStart(str, spaceChars);}, strings::default_space())});
        t.insert({"trim", OptionalStringArgument([](const std::string& str, const std::string& spaceChars) { return strings::Trim(str, spaceChars);}, strings::default_space())});
        t.insert({ "zfill", OptionalIntArgument([](const std::string& str, int count) { return strings::PadLeft(str, count, '0'); } , 3) });

        t.insert({"replace", StringStringArgument([](const std::string& arg, const std::string& lhs, const std::string& rhs) { return strings::Replace(arg, lhs, rhs); }) });
        t.insert({ "substr", IntIntArgument([](const std::string& arg, int lhs, int rhs) { return strings::Substring(arg, lhs, rhs);})});
        return t;
    }
}
