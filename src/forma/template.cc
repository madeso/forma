...
namespace forma
{
    struct Location { std::string File; int Line; int Offset};
    struct Error { Location Location; std::string Message};
    struct FuncArgument { Location Location; std::string Argument};

    /// parse arguments, return function call or error
    using FuncGenerator = std::function<std::pair<Func, std::vector<Error>> (Location call, std::vector<FuncArgument> arguments)>;

    /// apply dynamic std::string function and return result
    using Func = std::function< std::string (std::string arg)>;

    using FunctionMap = std::map<std::string, FuncGenerator>;

    template<typename TParent>
    struct Definition
    {
        using AttributeGetter = std::function<std::string (TParent)>;
        using BoolGetter = std::function<bool (TParent)>;
        using ChildGetter = std::function<std::pair<AttributeGetter, std::vector<Error>> (Node)>;

        std::map<std::string, AttributeGetter> attributes;
        std::map<std::string, BoolGetter> bools;
        std::map<std::string, ChildGetter> children;

        Definition<TParent>& AddVar(const std::string& name, AttributeGetter getter)
        {
            attributes[name] = getter;
            return *this;
        }

        Definition<TParent>& AddBool(const std::string& name, BoolGetter getter)
        {
            bools[name] = getter;
            return *this;
        }
        ...
        Definition<TParent>& AddList(std::string name, std::function<std::vector<TChild> (TParent)> childSelector, Definition<TChild> childDef)
        {
            children[name] = [](node)
            {
                ...
                return (parent => std::string.Join("", childSelector(parent).Select(getter)), NoErrors);
            };
            return *this;
        }

        static std::string SyntaxError(TParent) { return "Syntax error"; }

        internal (AttributeGetter, std::vector<Error>) Validate(Node node)
        {
            switch (node)
            {
                case Node.Text text:
                ...
                case Node.Attribute attribute:
                {
                    ...
                    {
                        return (SyntaxError, std::vector.Create(new Error(
                                attribute.Location,
                ...
                case Node.If check:
                {
                    ...
                    return (parent => getter(parent) ? body(parent) : std::string.Empty, NoErrors);
                }
                ...
                case Node.Group gr:
                {
                    ...
                    return (parent => std::string.Join("", getters.Select(x => x(parent)).ToList()), NoErrors);
                }
    ...
    async Task<(Func<T, std::string>, std::vector<Error>)> Parse<T>(std::string path, VfsRead vfs, FunctionMap functions, DirectoryInfo includeDir, Definition<T> definition)
    {
...
#if 0
    FunctionMap DefaultFunctions()
    {
        var culture = new CultureInfo("en-US", false);

        var t = new FunctionMap();
    ...
    struct Node
    {
        ...
        TextNode* get_Text(NodeOwner*) const;
        AttributeNode* get_Attribute(NodeOwner*) const;
        IterateNode* get_Iterate(NodeOwner*) const;
        IfNode* get_If(NodeOwner*) const;
        FunctionCallNode* get_FunctionCall(NodeOwner*) const;
        GroupNode* get_Group(NodeOwner*) const;
    };

    struct TextNode {std::string Value; Location Location};
    struct AttributeNode {std::string Name; Location Location};
    struct IterateNode {std::string Name; Node Body; Location Location};
    struct IfNode {std::string Name; Node Body; Location Location};
    struct FunctionCallNode {std::string Name; Func Function; Node Arg; Location Location};
    struct GroupNode {List<Node> Nodes; Location Location};

    TextNode* Node::get_Text(NodeOwner*) const { if(type != NodeType::Text) { return nullptr; } return &owner->Text[index]; }
    AttributeNode* Node::get_Attribute(NodeOwner*) const { if(type != NodeType::Attribute) { return nullptr; } return &owner->Attribute[index]; }
    IterateNode* Node::get_Iterate(NodeOwner*) const { if(type != NodeType::Iterate) { return nullptr; } return &owner->Iterate[index]; }
    IfNode* Node::get_If(NodeOwner*) const { if(type != NodeType::If) { return nullptr; } return &owner->If[index]; }
    FunctionCallNode* Node::get_FunctionCall(NodeOwner*) const { if(type != NodeType::FunctionCall) { return nullptr; } return &owner->FunctionCall[index]; }
    GroupNode* Node::get_Group(NodeOwner*) const { if(type != NodeType::Group) { return nullptr; } return &owner->Group[index]; }

    struct NodeOwner
    {
        std::vector<TextNode> Text;
        std::vector<AttributeNode> Attribute;
        std::vector<IterateNode> Iterate;
        std::vector<IfNode> If;
        std::vector<FunctionCallNode> FunctionCall;
        std::vector<GroupNode> Group;
    }

    private static std::vector<Error> NoErrors => std::vector<Error>.Empty;
    private static Location UnknownLocation => new(new std::string("unknown-file.txt"), -1, -1);

    private static std::string MatchStrings(std::string name, std::vector<std::string> candidates)
    {
        ...
        {
            var s = std::string.Join(", ", candidates);
            return $"[{s}]";
        }
    }
    ...
    private struct Token(TokenType Type, std::string Lexeme, Location Location, std::string Value);
    
    
    private struct ScannerLocation(int Line, int Offset, int Index);
    private static (std::vector<Token>, std::vector<Error>) Scanner(std::string file, std::string source)
    {
        ...
        ret.Add(new Token(TokenType.Eof, "", new Location(file, current.Line, current.Offset), std::string.Empty));
    ...
    private struct ParseError : Exception { }
    private static async Task<(Node, std::vector<Error>)> Parse(std::vector<Token> itok, FunctionMap functions, DirectoryInfo includeDir, std::string defaultExtension, VfsRead vfs)
    {
        ...
        FuncArgument ParseFunctionArg()
        {
            ...
            var arg = Advance();
            return new FuncArgument(arg.Location, arg.Value);
        }
