...
namespace forma
{
	namespace node
	{
		...
		Attribute::Attribute(std::string n, forma::Location l)
			...
			, Location(l)
		{}
		Attribute* Attribute::AsAttribute() { return this; }
		...
		Iterate::Iterate(std::string n, std::shared_ptr<Node> b, forma::Location l)
			...
			, Location(l)
		{}
		Iterate* Iterate::AsIterate() { return this; }
		...
		FunctionCall::FunctionCall(std::string n, Func f, std::shared_ptr<Node> a, forma::Location l)
			...
			, Location(l)
		{}
		FunctionCall* FunctionCall::AsFunctionCall() { return this; }

		Group::Group(std::vector<std::shared_ptr<Node>> n, forma::Location l)
			: Nodes(n)
			, Location(l)
		{}
		Group* Group::AsGroup() { return this; }
	}
	...
	std::vector<Token> TrimEmptyStartEnd(const std::vector<Token>& tokens)
	{
		...
		for(const auto& tok: tokens)
		{
			if (lastToken.has_value() && lastToken->Type == TokenType::BeginCode && tok.Type == TokenType::EndCode)
			{
	...
	std::vector<Token> TransformSingleCharsToKeywords(const std::vector<Token>& tokens)
	{
		...
		for(const auto& tok: tokens)
		{
			...
			if(tok.Type == TokenType::Slash && lastToken.has_value() && lastToken->Type == TokenType::BeginCode) {
				r.emplace_back(*lastToken);
				lastToken = tok.withType(TokenType::KeywordEnd);
				eatIdent = true;
			}
			else if(tok.Type == TokenType::Hash && lastToken.has_value() && lastToken->Type == TokenType::BeginCode) {
				r.emplace_back(*lastToken);
				lastToken = tok.withType(TokenType::KeywordRange);
			}
			else if(tok.Type == TokenType::QuestionMark && lastToken.has_value() && lastToken->Type == TokenType::BeginCode) {
				r.emplace_back(*lastToken);
	...
	struct Parser
	{
		...
		Parser(std::vector<Token> itok, std::unordered_map<std::string, FuncGenerator> f, DirectoryInfo* i, std::string d, VfsRead* v)
		...
		{
			std::string value = token.Type == TokenType::Text ? std::string{ "" } : Fmt{} << ": " << token.Lexeme;
		...
		void ParseNode(std::vector<std::shared_ptr<Node>>& nodes)
		{
			...
			case TokenType::BeginCode:
				...
				else if (Match(TokenType::KeywordInclude))
				{
					...
					if (vfs->Exists(file) == false)
					{
						ReportError(includeLocation, Fmt{} << "Unable to open file: tried " << firstFile << " and " << secondFile);
					}
					else
					{
			...
			break;
			default:
				throw ReportError(Peek().Location, Fmt{} << "Unexpected token " << TokenToMessage(Peek()));
			}
		}
		...
		{
			auto start = Peek().Location;
			std::shared_ptr<Node> node = std::make_shared<node::Attribute>(ExtractAttributeName(), start);
			...
			while (Peek().Type == TokenType::Pipe)
			{
				...
				{
					ReportError(name.Location, Fmt{} << "Unknown function named " << name.Value << ": " << MatchStrings(name.Value, GetKeys(functions)));
				}
			}
			nodes.emplace_back(node);
	...
	ParseResult Parse(std::vector<Token> itok, std::unordered_map<std::string, FuncGenerator> functions, DirectoryInfo* includeDir, std::string defaultExtension, VfsRead* vfs)
	{
